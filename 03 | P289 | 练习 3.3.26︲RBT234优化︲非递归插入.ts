...
import { BSTNode } from './03 | P266 | 练习 3.2.34︲线性符号表'
import { RedBlackBST234, RedBlackBSTNode } from './03 | P283 | 符号表︲自顶而下的 2-3-4 树'



export class RedBlackBST234While<K, V> extends RedBlackBST234<K, V>{
  constructor(array) {
    super(array)
  }
  ...
  rotateRightWp(chain: RedBlackBSTNode<K, V>[], _i: number) {
  ...
  rotateLeftWp(chain: RedBlackBSTNode<K, V>[], _i: number) {
    const len = chain.length
    const i = len - _i
    const a = chain[i]
